...
class MOCK_CONNECTION_PROXY : public CONNECTION_PROXY {
 public:
  ...
  DataSource* get_ds() { return this->ds; };
  ...
  MOCK_METHOD(bool, connect,
              (const char*, const char*, const char*, const char*, unsigned int, const char*, unsigned long));
...
class MOCK_MONITOR3 : public MONITOR {
 public:
  MOCK_MONITOR3(std::shared_ptr<HOST_INFO> host, std::chrono::milliseconds disposal_time)
      : MONITOR(host, nullptr, std::chrono::seconds{5}, disposal_time, nullptr, nullptr) {}
...
class MOCK_MONITOR_THREAD_CONTAINER : public MONITOR_THREAD_CONTAINER {
 public:
  ...
  void release_resources() { MONITOR_THREAD_CONTAINER::release_resources(); }

  MOCK_METHOD(std::shared_ptr<MONITOR>, create_monitor,
              (std::shared_ptr<HOST_INFO>, std::shared_ptr<CONNECTION_HANDLER>, std::chrono::seconds,
               std::chrono::milliseconds, DataSource*, bool));
};

class MOCK_MONITOR_CONNECTION_CONTEXT : public MONITOR_CONNECTION_CONTEXT {
 public:
  MOCK_MONITOR_CONNECTION_CONTEXT(std::set<std::string> node_keys, std::chrono::milliseconds failure_detection_time,
                                  std::chrono::milliseconds failure_detection_interval, int failure_detection_count)
      : MONITOR_CONNECTION_CONTEXT(nullptr, node_keys, failure_detection_time, failure_detection_interval,
...
class MOCK_MONITOR_SERVICE : public MONITOR_SERVICE {
 public:
  ...
  MOCK_METHOD(std::shared_ptr<MONITOR_CONNECTION_CONTEXT>, start_monitoring,
              (DBC*, DataSource*, std::set<std::string>, std::shared_ptr<HOST_INFO>, std::chrono::milliseconds,
...
class MOCK_RDS_CLIENT : public Aws::RDS::RDSClient {
 public:
  ...
  MOCK_METHOD(Aws::RDS::Model::DescribeDBClusterEndpointsOutcome, DescribeDBClusterEndpoints,
              (const Aws::RDS::Model::DescribeDBClusterEndpointsRequest&), (const));
};
...
class MOCK_SAML_HTTP_CLIENT : public SAML_HTTP_CLIENT {
 public:
  MOCK_SAML_HTTP_CLIENT(std::string host, int connect_timeout, int socket_timeout, bool enable_ssl)
      : SAML_HTTP_CLIENT(host, connect_timeout, socket_timeout, enable_ssl){};
...
class TEST_CUSTOM_ENDPOINT_PROXY : public CUSTOM_ENDPOINT_PROXY {
 public:
  TEST_CUSTOM_ENDPOINT_PROXY(DBC* dbc, DataSource* ds, CONNECTION_PROXY* next_proxy)
      : CUSTOM_ENDPOINT_PROXY(dbc, ds, next_proxy){};
  MOCK_METHOD(std::shared_ptr<CUSTOM_ENDPOINT_MONITOR>, create_custom_endpoint_monitor,
