...
TEST_F(MySQLProxyTest, FailureDetectionDisabled) {
    ...
    EXPECT_CALL(*mock_monitor_service, start_monitoring(_, _, _, _, _, _, _)).Times(0);
...
TEST_F(MySQLProxyTest, FailureDetectionEnabled) {
    ...
    EXPECT_CALL(*mock_monitor_service, start_monitoring(_, _, _, _, _, _, _)).WillOnce(Return(mock_context));
