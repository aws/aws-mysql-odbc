...
class CustomEndpointIntegrationTest : public BaseFailoverIntegrationTest {
 protected:
  ...
  std::string ENDPOINT_ID = "test-endpoint-1-" + std::chrono::steady_clock::now().time_since_epoch().count();
  Aws::RDS::RDSClientConfiguration client_config;
  Aws::RDS::RDSClient rds_client;
  Aws::RDS::Model::DBClusterEndpoint& endpoint_info;
  SQLHENV env = nullptr;
  SQLHDBC dbc = nullptr;
  ...
  void SetUp() override {
    ...
    builder.withPort(MYSQL_PORT).withLogQuery(true).withEnableFailureDetection(true);
  ...
  void wait_until_endpoint_available(const std::string& cluster_id) const {
    ...
    while (std::chrono::steady_clock::now() < end_time) {
      ...
      request.SetDBClusterIdentifier(ENDPOINT_ID);
      request.SetFilters({filter});
      const auto response = rds_client.DescribeDBClusterEndpoints(request);

      auto custom_endpoints = response.GetResult().GetDBClusterEndpoints();
      if (custom_endpoints.size() != 1) {
        std::this_thread::sleep_for(std::chrono::seconds(3));
      }

      endpoint_info = custom_endpoints[0];
...
TEST_F(CustomEndpointIntegrationTest, test_CustomeEndpointFailover) {
  connection_string = builder.withDSN(dsn)
                          .withServer(endpoint_info.GetEndpoint())
                          .withUID(user)
                          .withPWD(pwd)
                          .withDatabase(db)
