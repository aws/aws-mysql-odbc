...
class ConnectionString {
 public:
  ...
  std::string get_connection_string() const {
    ...
    if (is_set_enable_custom_endpoint_monitoring) {
      length += sprintf(conn_in + length, "ENABLE_CUSTOM_ENDPOINT_MONITORING=%d;", m_enable_custom_endpoint_monitoring ? 1 : 0);
    }
    if (is_set_should_wait_for_info) {
      length += sprintf(conn_in + length, "WAIT_FOR_CUSTOM_ENDPOINT_INFO=%d;", m_should_wait_for_info ? 1 : 0);
    }
    if (is_set_custom_endpoint_info_refresh_rate_ms) {
      length += sprintf(conn_in + length, "CUSTOM_ENDPOINT_INFO_REFRESH_RATE_MS=%d;", m_custom_endpoint_info_refresh_rate_ms);
    }
    if (is_set_wait_on_cached_info_duration_ms) {
      length += sprintf(conn_in + length, "WAIT_FOR_CUSTOM_ENDPOINT_INFO_TIMEOUT_MS=%ld;", m_wait_on_cached_info_duration_ms);
    }
    if (is_set_idle_monitor_expiration_ms) {
      length += sprintf(conn_in + length, "CUSTOM_ENDPOINT_MONITOR_EXPIRATION_MS=%ld;", m_idle_monitor_expiration_ms);
    }
 ...
 private:
  ...
  int m_failure_detection_time, m_failure_detection_timeout, m_failure_detection_interval, m_failure_detection_count,
      m_monitor_disposal_time, m_read_timeout, m_write_timeout;
  std::string m_auth_mode, m_auth_region, m_auth_host, m_secret_id;
  int m_auth_port, m_auth_expiration;
  bool m_enable_custom_endpoint_monitoring, m_should_wait_for_info;
  long m_custom_endpoint_info_refresh_rate_ms, m_wait_on_cached_info_duration_ms, m_idle_monitor_expiration_ms;
  ...
  bool is_set_failure_detection_time, is_set_failure_detection_timeout, is_set_failure_detection_interval,
      is_set_failure_detection_count;
  bool is_set_monitor_disposal_time;
  bool is_set_read_timeout, is_set_write_timeout;
  bool is_set_auth_mode, is_set_auth_region, is_set_auth_host, is_set_auth_port, is_set_auth_expiration,
      is_set_secret_id;
  bool is_set_enable_custom_endpoint_monitoring, is_set_should_wait_for_info;
  bool is_set_custom_endpoint_info_refresh_rate_ms, is_set_wait_on_cached_info_duration_ms, is_set_idle_monitor_expiration_ms;

  void set_dsn(const std::string& dsn) { m_dsn = dsn; }

  void set_server(const std::string& server) { m_server = server; }
  ...
  void set_enable_custom_endpoint_monitoring(const bool& enable_custom_endpoint_monitoring) {
    m_enable_custom_endpoint_monitoring = enable_custom_endpoint_monitoring;
    is_set_enable_custom_endpoint_monitoring = true;
  }

  void set_should_wait_for_info(const bool& wait_for_info) {
    m_should_wait_for_info = wait_for_info;
    is_set_should_wait_for_info = true;
  }

  void set_custom_endpoint_info_refresh_rate_ms(const long& custom_endpoint_info_refresh_rate_ms) {
    m_custom_endpoint_info_refresh_rate_ms = custom_endpoint_info_refresh_rate_ms;
    is_set_custom_endpoint_info_refresh_rate_ms = true;
  }

  void set_wait_on_cached_info_duration_ms(const long& wait_on_cached_info_duration_ms) {
    m_wait_on_cached_info_duration_ms = wait_on_cached_info_duration_ms;
    is_set_wait_on_cached_info_duration_ms = true;
  }

  void set_idle_monitor_expiration_ms(const long& idle_monitor_expiration_ms) {
    m_idle_monitor_expiration_ms = idle_monitor_expiration_ms;
    is_set_idle_monitor_expiration_ms = true;
  }
};

class ConnectionStringBuilder {
 public:
  ConnectionStringBuilder() { connection_string.reset(new ConnectionString()); }
  ...
  ConnectionStringBuilder& withEnableCustomEndpointMonitoring(const bool& enable_custom_endpoint_monitoring) {
    connection_string->set_enable_custom_endpoint_monitoring(enable_custom_endpoint_monitoring);
    return *this;
  }

  ConnectionStringBuilder& withShouldWaitForInfo(const bool& should_wait_for_info) {
    connection_string->set_should_wait_for_info(should_wait_for_info);
    return *this;
  }

  ConnectionStringBuilder& withCustomEndpointInfoRefreshRateMs(const long& custom_endpoint_info_refresh_rate_ms) {
    connection_string->set_custom_endpoint_info_refresh_rate_ms(custom_endpoint_info_refresh_rate_ms);
    return *this;
  }

  ConnectionStringBuilder& withWaitOnCachedInfoDurationMs(const long& wait_on_cached_info_duration_ms) {
    connection_string->set_wait_on_cached_info_duration_ms(wait_on_cached_info_duration_ms);
    return *this;
  }

  ConnectionStringBuilder& withIdleMonitorExpirationMs(const long& idle_monitor_expiration_ms) {
    connection_string->set_idle_monitor_expiration_ms(idle_monitor_expiration_ms);
    return *this;
  }
