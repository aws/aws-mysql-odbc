...
DECLARE_TEST(bug12824839)
{
  ...
  for (int i = 0; i < 2; ++i)
  {
    ...
    rc = SQLPrepare(hstmt1, (SQLCHAR*)"SELECT * from any_non_existing_table", SQL_NTS);
    ...
    ok_stmt(hstmt1, SQLColumns(hstmt1, (SQLCHAR*)"test", SQL_NTS, NULL, 0, (SQLCHAR*)"b12824839",
                              SQL_NTS, NULL, 0));
    ...
    ok_stmt(hstmt1, SQLColumns(hstmt1, (SQLCHAR*)"test", SQL_NTS, NULL, 0, NULL,
                              SQL_NTS, NULL, 0));
    ok_stmt(hstmt1, SQLRowCount(hstmt1, &row_count));
...
DECLARE_TEST(sqlcolumns_nodbselected)
{
  ...
  for (int i = 0; i < 2; ++i)
  {
    is(OK == alloc_basic_handles_with_opt(&henv1, &hdbc1, &hstmt1,
      USE_DRIVER, NULL, NULL, (SQLCHAR*)"", (SQLCHAR*)conn_opt[i]));
