...
SQLRETURN do_query(STMT *stmt, char *query, SQLULEN query_length)
{
    ...
    {
      native_error= stmt->dbc->mysql_proxy->stmt_bind_param(stmt->ssps,
    ...
    if (native_error)
    {
      ...
      if (error_code)
      {
          ...
          // For some errors - translating to more appropriate status
          translate_error(stmt->error.sqlstate, MYERR_S1000, error_code);
      }
      else
      {
          MYLOG_STMT_TRACE(stmt, stmt->dbc->error.message);
          stmt->set_error(stmt->dbc->error.sqlstate,
              stmt->dbc->error.message,
              stmt->dbc->error.native_error);
      }
...
exit:
    if (trigger_failover_upon_error && error == SQL_ERROR) {
      ...
      if (stmt->dbc->fh->trigger_failover_if_needed(stmt->error.sqlstate, error_code, error_msg))
