...
void MYSQL_PROXY::close_socket() {
    MYLOG_TRACE(init_log_file(), "Closing socket");
    int ret = 0;
    if (mysql->net.fd != INVALID_SOCKET && (ret = shutdown(mysql->net.fd, SHUT_RDWR))) {
       MYLOG_TRACE(init_log_file(), "shutdown() with return code: %d, error message: %s,", ret, strerror(socket_errno));
    }
    ...
    if (mysql->net.fd != INVALID_SOCKET && (ret = ::closesocket(mysql->net.fd))) {
        MYLOG_TRACE(init_log_file(), "closesocket() with return code: %d, error message: %s,", ret, strerror(socket_errno));
    }
}
