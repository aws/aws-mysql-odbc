                    ...
                    SQLPOINTER ValuePtr, SQLINTEGER StringLengthPtr)
{
  ...
  switch (Attribute)
  {
    ...
    case SQL_ATTR_AUTOCOMMIT:
      ...
      else if (dbc->mysql == nullptr || !dbc->mysql->is_connected())
      {
    ...
    case SQL_ATTR_CURRENT_CATALOG:
      {
        ...
        if (dbc->mysql != nullptr && dbc->mysql->is_connected())
        {
          if (dbc->mysql->select_db((char*) db))
          {
            set_conn_error(dbc, MYERR_S1000, dbc->mysql->error(), dbc->mysql->error_code());
            return SQL_ERROR;
          }
        }
        dbc->database = db ? db : "";
    ...
    case SQL_ATTR_TXN_ISOLATION:
      if (dbc->mysql == nullptr || !dbc->mysql->is_connected())  /* no connection yet */
      {
                    ...
                    SQLPOINTER num_attr)
{
  ...
  case SQL_ATTR_CONNECTION_DEAD:
    ...
    if (dbc->need_to_wakeup != 0 && wakeup_connection(dbc)
      || dbc->need_to_wakeup == 0 && dbc->mysql->ping() &&
        is_connection_lost(dbc->mysql->error_code()))
  ...
  case SQL_ATTR_PACKET_SIZE:
    *((SQLUINTEGER*)num_attr) = dbc->mysql->get_max_packet();
    break;
    ...
    */
    if (!dbc->txn_isolation)
    {
      ...
      */
      ...
      if (is_minimum_version(dbc->mysql->get_server_version(), "8.0"))
