...
class SLIDING_EXPIRATION_CACHE {
 public:
  class CACHE_ITEM {
   ...
   public:
    CACHE_ITEM(V item, std::chrono::system_clock::time_point expiration_time)
   ...
   protected:
    bool should_clean_up() {
      if (should_dispose_func != nullptr) {
        return std::chrono::system_clock::now() > expiration_time && should_dispose_func->should_dispose(this->item);
      }
      return false;
    }

    CACHE_ITEM with_extend_expiration(long item_expiration_nanos) {
      this->expiration_time = std::chrono::system_clock::now() + std::chrono::nanoseconds(item_expiration_nanos);
      return this;
    }
  };
 ...
 protected:
  ...
  const std::atomic<std::chrono::system_clock::time_point> clean_up_time_nanos;
