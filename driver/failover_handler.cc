...
SQLRETURN FAILOVER_HANDLER::init_connection() {
    ...
    if (ds->opt_ENABLE_CLUSTER_FAILOVER) {
        ...
        if (!ds->opt_FAILOVER_MODE.is_set()) {
...
void FAILOVER_HANDLER::init_cluster_info() {
    ...
    if (!hp_str.empty()) {
        unsigned int port = ds->opt_PORT.is_set() ? ds->opt_PORT : MYSQL_PORT;
...
bool FAILOVER_HANDLER::should_connect_to_new_writer() {
    ...
    ds->opt_SERVER.set_remove_brackets((SQLWCHAR*) writer_host.c_str(), writer_host.size());
